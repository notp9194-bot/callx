...
<body>
  ...
  <script>
    ...
    // Turant app open karne ki koshish
    function tryOpen() {
      opened = true;
      window.location.href = appUrl;

      // 2 sec baad agar page still here hai = app nahi khula
      setTimeout(function() {
...
app.get("/profile/:uid", (req, res) => {
  ...
  res.send(deepLinkPage(
    `callx://profile/${uid}`,
    `https://callx-server.onrender.com/profile/${uid}`,
    "Profile",
    "Is user ka profile dekhen CallX app mein"
  ));
});
